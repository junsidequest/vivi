...
function PreparedPage({entry,visible,onReady,onError}){
  ...
  useEffect(()=>{
    ...
    Promise.all(images.map(image=>image.decode().catch(()=>{}))).then(()=>{if(!cancelled)ready()})
    return()=>{cancelled=true}
  },[entry,ready])
  const Page=entry.Page
  return <div ref={host} className={`mobile-site-page${visible?' is-visible':''}`} aria-hidden={!visible} inert={!visible}>
    <Page externalLoading pageVisible={visible} onLoadReady={ready} onLoadError={onError}/>
  </div>
}
export default function MobileSite(){
  ...
  const ready=useCallback(async entry=>{
    ...
    shownRef.current=entry;setShown(entry);setPending(null)
  ...
  return <div className="mobile-site">
    ...
    {pending&&<PreparedPage key={pending.id} entry={pending} visible={false} onReady={ready} onError={()=>{setPending(null);setError(true)}}/>}
